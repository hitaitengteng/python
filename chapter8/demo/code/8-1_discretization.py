...
datafile = 'C:/Users/att/Desktop/python_practice_of_data_analysis_and_mining-master/chapter8/demo/data/data.xls'  # 待聚类的数据文件
processedfile = 'C:/Users/att/Desktop/python_practice_of_data_analysis_and_mining-master/chapter8/demo//tmp/data_processed.xls'  # 数据处理后文件
...
if __name__ == '__main__':  # 判断是否主窗口运行，如果是将代码保存为.py后运行，则需要这句，如果直接复制到命令窗口运行，则不需要这句。
    for i in range(len(keys)):
        ...
        #r[t#ypelabel[keys[i]]] = pd.rolling_mean(
            #r[typelabel[keys[i]]], 2)  # rolling_mean()用来计算相邻2列的均值，以此作为边界点。
        r[typelabel[keys[i]]][1] = 0.0 # 这两句代码将原来的聚类中心改为边界点。
        result = result.append(r.T)

    result = result.sort_index(axis=1)  # 以Index排序，即以A,B,C,D,E,F顺序排
